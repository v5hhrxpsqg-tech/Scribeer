...
# Hier maken we het geheugen aan
...
if uploaded_file:
    # STAP A: Verwerk de audio alleen als er nog GEEN tekst in het geheugen zit
    ...
    # STAP B: Als er tekst in het geheugen zit, toon deze dan (ook na een verversing!)
        ...
        if not st.session_state.ingelogd:
            ...
            st.markdown("""
                <div style="background-color:#f0f2f6; padding:20px; border-radius:10px; border:2px solid #ff4b4b; margin-top:20px;">
                    ...
                    <p>Maak een gratis account aan om alles te zien en te downloaden.</p>
                </div>
            """, unsafe_allow_html=True)
            
            if st.button("👉 Maak een gratis account"):
                st.session_state.ingelogd = True # Trucje: we 'faken' de inlog even
                st.rerun() # Pagina ververst, maar final_text blijft behouden!
